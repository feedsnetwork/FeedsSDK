...
class Channel implements ChannelHandler {
    ...
    */
    ...
    // 需订阅才能调用 同步feeds api //
    public async queryLikeByChannel(targetDid: string): Promise<Likeinfo[]> {
        try {
            const params = {
                "channel_id": this.getChannelInfo().getChannelId(),
                "status": 0 // available
            }
            const results = await this.vault.callScript(scripts.SCRIPT_QUERY_LIKE_BY_CHANNEL, params, targetDid, this.context.getAppDid())
            logger.debug("query like by channel  success: ", results)
            const result = results.find_message.items
            let likes = []
            result.forEach(item => {
