...
export class Comment {
    ...
    */
    ...
    // 同步feeds api 
    public async queryLikeById(commentId: string): Promise<any> {
        try {
            const params = {
                "channel_id": this.getCommentInfo().getChannelId(),
                "post_id": this.getCommentInfo().getPostId(),
            ...
            const results = await this.vault.callScript(scripts.SCRIPT_QUERY_LIKE_BY_ID, params, this.getCommentInfo().getTargetDidDid(), this.context.getAppDid())
